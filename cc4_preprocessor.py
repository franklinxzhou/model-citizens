...
# Changed from cc3_preprocessor.py

# History: cc_preprocessor --> cc3_preprocessor --> THIS preprocessor
# cc2_preprocessor is a unsuccessful version
...
class Preprocessor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for col in ['vehicle_color', 'channel', 'accident_site', 'zip3']:
            if col in df_fe.columns:
                df_fe[col] = self._cap_rare(df_fe[col], min_count=100)
    ...
    def _clean_data(self, df):
        ...
        if 'zip_code' in df_fe.columns:
            df_fe['zip3'] = df_fe['zip_code'].astype(str).str[:3]

        if 'policy_report_filed_ind' in df_fe.columns:
            df_fe['policy_report_filed_ind'] = df_fe['policy_report_filed_ind'].map(
                {'Y': 1, 'N': 0, 1: 1, 0: 0}
            ).fillna(0).astype(int)
    ...
    def _engineer_new_interactions(self, df):
        """Creates new interaction features based on importance analysis."""
        df_fe = df.copy()
        
        # Check if the target map for 'accident_site' was learned in 'fit'
        if 'accident_site' in self.target_mean_maps_:
            # Get the learned map
            accident_site_map = self.target_mean_maps_['accident_site']
            
            # Apply the encoding. 
            # Use global_mean_ for any unseen categories during transform.
            accident_site_enc = df_fe['accident_site'].map(accident_site_map).fillna(self.global_mean_)
            
            # Create the new interaction feature
            # We fillna(0) for liab_prct just in case, to not lose rows
            df_fe['liab_prct_x_accident_site_enc'] = df_fe['liab_prct'].fillna(0) * accident_site_enc
        
        return df_fe
    
    def _cap_rare(self, s, min_count=100):
        vc = s.value_counts(dropna=False)
        rare = vc[vc < min_count].index
        return s.where(~s.isin(rare), 'Other_Rare')